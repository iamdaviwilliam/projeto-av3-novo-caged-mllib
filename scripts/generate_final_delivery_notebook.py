...
def main():
    print("=== MONTAGEM DO NOTEBOOK FINAL COMPLETO COM SEÇÕES 1 A 19 (ENTREGA FINAL) ===")
    ...
    cells = [
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 17. Conclusão do Projeto\n",
                "\n",
                "### 17.1 Síntese do Estudo\n",
                ...
                "\n",
                "### 17.2 Principais Achados Técnicos\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 18. Resumo para Defesa Oral\n",
                "\n",
                "Mapa visual de tópicos para suporte à apresentação oral do projeto:\n",
                "\n",
                ...
                "- **Principais Limitações:** Análise restrita ao mercado formal CLT, abrangência geográfica do Nordeste e split aleatório em série temporal."
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": [
                "## 19. Checklist de Defesa — Conceitos Chave\n",
                "\n",
                "Definições concisas dos conceitos técnicos utilizados ao longo do projeto:\n",
                "\n",
                "1. **`StringIndexer`:** Transformador do PySpark MLlib que mapeia categorias textuais não ordenadas em índices numéricos inteiros para processamento computacional.\n",
                "2. **`OneHotEncoder`:** Transformador que converte índices categóricos em vetores binários esparsos, impedindo que modelos atribuam relação ordinal arbitrária a categorias nominais.\n",
                "3. **`VectorAssembler`:** Transformador que consolida múltiplos atributos numéricos e vetores de recursos em um único vetor denso/esparso denominado `features`.\n",
                "4. **`Pipeline`:** Estrutura de encadeamento do PySpark MLlib que sequencia etapas de pré-processamento e estimadores de modelagem em um fluxo único e reprodutível.\n",
                "5. **`randomSplit`:** Método de divisão estocástica do DataFrame em subconjuntos independentes (ex: 70% Treino e 30% Teste) controlado por uma semente aleatória (`seed`).\n",
                "6. **Data Leakage:** Vazamento de informação do futuro ou da variável resposta para o conjunto de atributos preditores no momento do treinamento.\n",
                "7. **AUC-ROC:** Métrica de área sob a curva caracteristicamente operada pelo receptor que avalia a capacidade discriminativa global do modelo entre 0,5 e 1,0.\n",
                "8. **Accuracy:** Proporção total de classificações corretas (positivas e negativas) sobre o total de observações avaliadas.\n",
                "9. **Precision:** Proporção de verdadeiros positivos em relação ao total de instâncias classificadas como positivas pelo modelo ($\\frac{TP}{TP + FP}$).\n",
                "10. **Recall:** Proporção de verdadeiros positivos capturados em relação ao total real de instâncias positivas no conjunto de dados ($\\frac{TP}{TP + FN}$).\n",
                "11. **F1-Score:** Média harmônica entre Precision e Recall, proporcionando um indicador sintético equilibrado da performance na classe positiva.\n",
                "12. **Matriz de Confusão:** Tabela de contingência $2 \\times 2$ que demarca os quantitativos de Verdadeiros Negativos (TN), Falsos Positivos (FP), Falsos Negativos (FN) e Verdadeiros Positivos (TP).\n",
                "13. **Logistic Regression:** Algoritmo linear de classificação probabilística baseado na função logística (sigmoide) para modelar os log-odds da classe positiva.\n",
                "14. **Random Forest:** Algoritmo de ensemble baseado na combinação de múltiplas árvores de decisão independentes construídas com amostragem de dados e atributos.\n",
                "15. **`featureImportances`:** Vetor atribuído às árvores de decisão que quantifica a redução acumulada de impureza (Gini/Variância) proporcionada por cada atributo.\n",
                "16. **Coeficientes:** Vetor de pesos da Regressão Logística que mensura a variação nos log-odds da variável resposta para o aumento unitário de um atributo.\n",
                "17. **`TrainValidationSplit`:** Módulo do PySpark MLlib para otimização de hiperparâmetros que avalia grades de parâmetros em uma única divisão interna de treino/validação.\n",
                ...
                "19. **Censura à Direita:** Exclusão de observações temporais finais cujo horizonte futuro (janela de 6 meses) não pôde ser completamente observado."
            ]
        }
    ]
